...
class LLMClient:
    ...
    def generate_answer(self, question: str, context: str, ministry: str = None) -> str:
        try:
            ...
            if context_docs:
                response = self._generate_structured_response(question, context_docs, ministry)
            else:
                response = self._generate_simple_response(question, ministry)
                
            logger.info(f"Generated response for question about {ministry or 'general topic'}")
            return response
    ...
    def generate_response_with_docs(self, question: str, context_docs: List[Dict[str, Any]], ministry: str = None) -> str:
        try:
    ...
    async def generate_response_async(self, question: str, context: List[Dict[str, Any]], ministry: str) -> str:
        try:
            ...
            formatted_response = self._format_parliamentary_response(response.text, context)
    ...
    def _generate_structured_response(self, question: str, context_docs: List[Dict[str, Any]], ministry: str = None) -> str:
        ...
        try:
            ...
            formatted_response = self._format_parliamentary_response(response.text, context_docs)
            return formatted_response
...
"""
    ...
    def _construct_enhanced_prompt(self, question: str, context_docs: List[Dict[str, Any]], ministry: str = None) -> str:
        """Construct detailed prompt for Parliamentary Q&A with rich metadata"""
        try:
            context_parts = []
...
"""
            ...
            # Much more detailed and parliamentary-specific prompt
            prompt = f"""
...
"""
    ...
    def _call_gemini_api(self, prompt: str):
        """Make API call to Gemini with optimized settings for parliamentary Q&A"""
        try:
            generation_config = {
                "temperature": 0.7,  # Lower temperature for more factual, consistent responses
                "top_p": 0.8,
                "top_k": 40,
                "max_output_tokens": 3000,  # Increased for more comprehensive answers
            }
    ...
    def _format_parliamentary_response(self, text: str, context_docs: List[Dict[str, Any]]) -> str:
        """Format response with source citations for parliamentary Q&A system"""
        try:
            ...
            # Only add source citations if we have meaningful metadata (not generic "Unknown" values)
            if context_docs and not self._is_irrelevant_response(formatted_text):
                meaningful_citations = []
                
                for i, doc in enumerate(context_docs[:3], 1):  # Top 3 sources for citations
                    metadata = doc.get("metadata", {})
                    date = metadata.get("date", "Unknown date")
                    session = metadata.get("session", "Unknown session")
                    source = metadata.get("filename", metadata.get("source", "Unknown source"))
                    
                    # Only add citation if we have meaningful metadata (not all "Unknown" values)
                    if (date != "Unknown date" or 
                        session not in ["Unknown session", "4"] or  # "4" is also a generic default
                        source not in ["Unknown source", "parliamentary_document"]):
                        
                        meaningful_citations.append(
                            f"[{i}] Parliamentary record from Session {session}, dated {date} (Source: {source})"
                        )
                
                # Only add sources section if we have meaningful citations
                if meaningful_citations:
                    formatted_text += f"\n\n**Sources:**\n" + "\n".join(meaningful_citations)
    ...
    def _is_irrelevant_response(self, text: str) -> bool:
        """Enhanced check if the response indicates the question was irrelevant"""
    ...
    def test_connection(self) -> bool:
        ...
        try:
            test_response = self._call_gemini_api("Respond with 'Connection successful' if you can process this message.")
